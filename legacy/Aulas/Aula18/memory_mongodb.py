...
def limpar_mensagens_vazias(session_id: str) -> None:
    """Remove mensagens com conteúdo vazio da sessão ativa."""
    doc_id = _sessoes_ativas.get(session_id)
    if not doc_id:
        return

    # 1. Remove mensagens vazias
    col_sessoes.update_many(
        {"_id": doc_id},
        {"$pull": {"mensagens": {"content": {"$regex": r"^\s*$"}}}},
    )
    
    # 2. Verifica se ainda há mensagens
    doc = col_sessoes.find_one({"_id": doc_id}, {"mensagens": 1})
    
    # 3. Se não houver mensagens, remove a sessão
    if doc and not doc.get("mensagens"):
        col_sessoes.delete_one({"_id": doc_id})
        # Remove da lista de sessões ativas também
...
def encerrar_sessao(session_id) -> str:
    ...
    #limpando mensagens vazias antes de gerar resumo
    limpar_mensagens_vazias(session_id)
...
def recuperar_historico(session_id: str) -> list[dict]:
    """
    Recupera o histórico de mensagens da sessão ativa.
    Retorna uma string concatenada das mensagens.
    """
    doc_id = _sessoes_ativas.get(session_id)
    ...
    doc = col_sessoes.find({"_id": doc_id}).limit(3).sort("atualizada_em", -1).next()
    if not doc or not doc.get("mensagens"):
        return ""
    return _formatar_conversa(doc["mensagens"])
